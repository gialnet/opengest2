...
function Header()
{
	...
	$this->Cell(90);
	//Título
	$this->Cell(10,30,'Notarios a fecha de: '.date("d/m/y"));
	//Salto de línea
	$this->Ln(20);
...
function FancyTable($header, $stid)
{
	...
	//Cabecera
	$w=array(15,65,65,42,40,10,10,10,5,15);
...
while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) 
	{
		$this->Cell($w[0],10,$row['NIF'],'LR',0,'L',$fill);
		$this->Cell($w[1],10,$row['NOMBRE'],'LR',0,'L',$fill);	
		$this->Cell($w[2],10,$row['DIRECCION'],'LR',0,'L',$fill);
		$this->Cell($w[3],10,$row['POBLACION'],'LR',0,'L',$fill);
		$this->Cell($w[4],10,$row['PROVINCIA'],'LR',0,'L',$fill);
		$this->Cell($w[5],10,$row['COD_POSTAL'],'LR',0,'L',$fill);
		$this->Cell($w[6],10,$row['ENTIDAD'],'LR',0,'L',$fill);
		$this->Cell($w[7],10,$row['OFICINA'],'LR',0,'L',$fill);
		$this->Cell($w[8],10,$row['DC'],'LR',0,'L',$fill);
		$this->Cell($w[9],10,$row['CUENTA'],'LR',0,'L',$fill);
...
$pdf->SetFont('Times','',15);
	...
	$header=array('NIF','Nombre','Dirección','Población','Provincia','CP','Entidad','Oficina','DC','Cuenta');	
