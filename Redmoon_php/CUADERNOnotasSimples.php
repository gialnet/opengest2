...
$nomfi='C:\Temp\cuadernoNS'.date("ymd").date("His").'.txt';
$_SESSION["nomfi"]=$nomfi;
//nombre que se le va a dar al cuaderno al subirlo a la nube se le
//añadirá la fecha y hora para que el nombre sea unico y se pueda 
...
function PintarRegCabecera($fi,$xCuenta){
	...
	//HABRA QUE PONER EN EL INTERFAZ DESDE QUE CUENTA QUIEREN ENVIAR EL DINERO Y SI QUIEREN AÑADIR
...
function PintarRegBeneficiarios($fi,$xCIF,$xIDCuaderno){
	...
	$sql3="select importe from config_ns";
	$stid3 = oci_parse($conn, $sql3);
	$r = oci_execute($stid3, OCI_DEFAULT);
	$row3 = oci_fetch_array($stid3, OCI_ASSOC+OCI_RETURN_NULLS);	
	
	$xImporte=$row3['IMPORTE'];
	
	//HABRA QUE PONER EN EL INTERFAZ DESDE QUE CUENTA QUIEREN ENVIAR EL DINERO Y SI QUIEREN AÑADIR
	//OTRA CUENTA O SI QUIEREN BORRARLA
	$sql="SELECT NIF, APENOMBRE,ENTIDAD,OFICINA,DC,CUENTA,DOMICILIO,CODPOSTAL,POBLACION
 	FROM LGS_NOTAS_SIMPLES
 	WHERE ESTADO='NO' and IDCUADERNO=:xIDCuaderno ";
	$stid2 = oci_parse($conn, $sql);
	...
	while($row = oci_fetch_array($stid2, OCI_ASSOC+OCI_RETURN_NULLS)){
		...
		$xOficinaBeneficiario=$row['ODICINA'];
		$xNCuentaBeneficiario=$row['CUENTA'];
...
function do_save_oracle($nomfi,&$xIDCuaderno)
{
	...
	$xFileName ='cuadernosNS/'.$xNombreComun;
	$url_oracle = 'http://cg-servicios.s3.amazonaws.com/'.$xFileName;

		$conn = db_connect();
// Nombre de procedimiento a ejecutar y sus parametros
$sql = 'begin AddCuaderno34NS(:url_oracle,:xIDCuaderno); end;';
...
// los registro de los pagos incluso se podría dividir en dos consultas una para los datos del colaborador 
// y otra para los importes.
//
// Una vez generado el soporte tendremos que actualizar la fase de los expedientes afectados y añadir su
// correspondiente asiento en el seguimiento.
//
// (Opcional) Sería también una buena idea tener la posibilidad de deshacer un cuaderno una vez realizado,
// poder volver a la situación inicial, los humanos cometemos errores y por lo tanto hay que darles oportunidad
// de rectificar. También es debido a que el fichero una vez realizado se envia
// a traves de la pagina Web de Caja Granada de banca electronica, es decir en otra aplicación. Podría ocurrir
...
//
// Se necesita en el interfaz de usuario una opción donde el usuario nos indique que el archivo ha sido
...
// De tal suerte que podemos iniciar con unos objetivos básicos e ir creciendo en funcionalidad, solo 
// con cambiar el PL/SQL sin afectar al interfaz de usuario, que se mantendrá constante en el tiempo
//
// Es importante ser muy ambicioso en los planteamientos iniciales, para ver más alla de lo que en realidad
// vamos desarrollar inicialmente. Con esta visión estamos sembrado el futuro de posibles mejoras. En caso 
// contrario cualquier mejora que nos propongan en el futuro sería inviable o dificil de acometer ya que
// no estaba prevista y en ocasiones es mejor reescribir todo el código, por deficiencias en la planificación
//
