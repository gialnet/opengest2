...
$pdf->Line(202, 55, 202,75);
	...
	$pdf->Cell(40,10,'Nº de Oficina: '.$oficina,0,0);
	...
	$pdf->Ln(6);
	$pdf->Cell(40,10,'Móvil: '.$row3['MOVIL'],0,0);
	$pdf->Cell(40,10,'Teléfono: '.$row3['TELEFONO'],0,0);
	$pdf->Ln(20);
...
while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) 
	{
		...
		if($row['IMPORTE']!=0)
		$this->Cell($w[2],6,number_format($row['IMPORTE']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		else 
		$this->Cell($w[2],6,' ','LR',0,'R',$fill);
...
while ($row4 = oci_fetch_array($stid4, OCI_ASSOC+OCI_RETURN_NULLS)) 
	{
		...
		$this->Cell($w[1],6,number_format($row4['OPR_IMPORTE']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Cell($w[2],6,number_format($row4['IMP_NOTARIA']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Cell($w[3],6,number_format($row4['IMP_IMPUESTO']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Cell($w[4],6,number_format($row4['IMP_REGISTRO']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Cell($w[5],6,number_format($row4['IMP_GESTORIA']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Cell($w[6],6,number_format($row4['TOTALPRESUPUESTO']).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
		$this->Ln();
		$fill=!$fill;
		$Total+=$row4['TOTALPRESUPUESTO'];
	...
	$this->Cell($w[4],6,'','LR',0,'R',$fill);
	$this->Cell($w[5],6,'','LR',0,'R',$fill);
	$this->Cell($w[6],6,number_format($Total).iconv('UTF-8', 'windows-1252', " â‚¬"),'LR',0,'R',$fill);
	$this->Ln();
	$this->Cell(array_sum($w),0,'','T');
	/*
	foreach($data as $row)
...
$pdf->Line(202, 55, 202,75);
	...
	$pdf->Cell(40,10, iconv('UTF-8', 'windows-1252', 'NÂº de Oficina: ').$oficina,0,0);
	...
	$pdf->Ln(6);
	$pdf->Cell(40,10,iconv('UTF-8', 'windows-1252', 'MÃ³vil: ').$row3['MOVIL'],0,0);
	$pdf->Cell(40,10,iconv('UTF-8', 'windows-1252', 'TelÃ©fono: ').$row3['TELEFONO'],0,0);
	$pdf->Ln(20);
	$pdf->MultiCell(0,5,'Importe de las Provisiones de Fondos');
	...
	$header2=array('Tipo de Acto','Importe', 'NotarÃ­a','Impuesto','Registro', 'GestiÃ³n', 'Total ProvisiÃ³n');
	...
	$pdf->MultiCell(0,5,'Listado del Seguimiento');
	$header=array('DescripciÃ³n' ,'Fecha','Importe','Tipo');	
...
// descomentar para grabar estadisticas de mediciÃ³n de caudal

// A configurar en cada mÃ³dulo =======================
$time_end = microtime(true);
$time = $time_end - $time_start;
