...
where to_char(fecha_apertura,'YYYY')=2006
...
while($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)){
	$vec_solicitudes06[$i]= (int)$row['SOLICITUDES'];
	$vec_anio06[$i]=$row['ANIO'];
	$i++;
}
...
while($row = oci_fetch_array($stid2, OCI_ASSOC+OCI_RETURN_NULLS)){
	$vec_solicitudes07[$b]= (int)$row['SOLICITUDES'];
	
	$vec_anio07[$b]=$row['ANIO'];
	$b++;
}
...
$title = new title( 'Comparativa numero de solicitudes por mes 06/07');


$bar1 = new bar();
$bar1->set_values( $vec_solicitudes06 );
$bar1->set_key( '2006', 12 );
$bar1->set_tooltip( '#bottom#<br>Valor:#val#' );

$bar2 = new bar();
$bar2->set_values( $vec_solicitudes07 );
