...
function VerExpe(xFila){
	...
	location="Oficinas.php?"+url_parametros;
}
...
function sendMail(xFila){
	// cells[2] es la direcci�n de email
	var idFila="ofila"+xFila;
	var oCelda=document.getElementById(idFila).cells[3];
	...
	for(x=0; x<=ArrayColumnas.length-2; x+=4)
		...
		j++;
			...
			cellText =  row.insertCell(0);
  			...
  			textNode = document.createTextNode(tbl.rows.length - 1);
  			cellText.appendChild(textNode);
  			cellText.innerHTML='<img src="redmoon_img/imgTablas/telefono.png" alt="Llamar a la Oficina" title="Llamar a la Oficina"  onclick="ExpedienteRevisado('+j+')"  id="oFase'+j+'">';

  			// VER LOS DATOS DE LA OFINA Y PODER MODIFICARLOS
  			cellText =  row.insertCell(5);
  			textNode = document.createTextNode(tbl.rows.length - 1);
  			cellText.appendChild(textNode);
  			cellText.innerHTML='<img src="redmoon_img/imgTablas/ver.png" alt="Ver datos de la Oficina" title="Ver Datos de la Oficina" onclick="VerExpe('+j+')"  id="oFase'+j+'">';

  			// ENVIAR CORREO
  			cellText =  row.insertCell(6);
  			textNode = document.createTextNode(tbl.rows.length - 1);
  			cellText.appendChild(textNode);
  			cellText.innerHTML='<img src="redmoon_img/imgTablas/mail.png" alt="Enviar un eMail a la Oficina" title="Enviar un eMail a la Oficina" onclick="sendMail('+j+')" id="oFase'+j+'">';
...
<div id="AniadirOficina">
<img src="redmoon_img/imgDoc/AadirOficina.png" align="middle" onclick="AniadirOficina()"/>
</div>
...
<table width="750" border="0" id="oTabla">
  <tr bgcolor="#C6DFEC" id="oFila0" style="color:#3C4963; cursor:default">
    ...
    <td width="30%">Poblaci&oacute;n</td>
